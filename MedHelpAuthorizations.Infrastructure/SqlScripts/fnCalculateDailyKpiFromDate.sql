...
GO
IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[IntegratedServices].[fnCalculateDailyKpiFromDate]') 
    --AND xtype IN (N'FN', N'IF', N'TF')
)
    DROP FUNCTION [IntegratedServices].[fnCalculateDailyKpiFromDate]
GO
CREATE OR ALTER     FUNCTION [IntegratedServices].[fnCalculateDailyKpiFromDate](
...
(
    ...
    DaysOperated AS (
        SELECT COUNT(*) AS DaysOperated,
               SUM(CASE WHEN D.Date >= @ClaimBilledFrom THEN 1 ELSE 0 END) AS DaysInRange
        FROM Dates AS D

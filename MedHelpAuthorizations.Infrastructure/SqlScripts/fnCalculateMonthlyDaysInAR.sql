...
--IF EXISTS (
--    SELECT * FROM sysobjects WHERE id = object_id(N'[IntegratedServices].[fnGetInitialClaimsSummary]') 
--    --AND xtype IN (N'FN', N'IF', N'TF')
--)
--    DROP FUNCTION [IntegratedServices].[fnGetInitialClaimsSummary]
--GO

CREATE OR ALTER FUNCTION [IntegratedServices].[fnCalculateMonthlyDaysInAR](
...
BEGIN
    ...
    RIGHT JOIN dbo.Clients as cl on cl.Id = c.ClientId
	RIGHT JOIN dbo.UserClients as uc on uc.ClientId = cl.Id 
    WHERE ClaimBilledOn >= @StartDate AND ClaimBilledOn <= @EndOfMonth
	AND (@ClientId IS NULL OR @ClientId <= 0 OR c.ClientId IN (SELECT ClientId FROM @tempUserAssignedClients))
	AND c.IsDeleted = 0 

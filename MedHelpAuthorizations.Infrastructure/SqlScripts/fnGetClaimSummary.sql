...
IF EXISTS (
    SELECT * FROM sysobjects WHERE id = object_id(N'[IntegratedServices].[fnGetClaimSummary]') 
    AND xtype IN (N'FN', N'IF', N'TF')
)
    DROP FUNCTION [IntegratedServices].[fnGetClaimSummary]
GO

CREATE FUNCTION [IntegratedServices].[fnGetClaimSummary](
@ClientId int
...
RETURN
	...
	,SUM(
            CASE 
                WHEN cst.ClaimStatusTypeId = 1 THEN t.LineItemPaidAmount
            END
        ) AS 'PaymentTotals',
        COUNT(DISTINCT(
            CASE 
                WHEN cst.ClaimStatusTypeId = 1 THEN c1.ClaimLevelMd5Hash
            END
        )) AS 'PaymentVisits',
        SUM(
            CASE 
        ...
        SUM(
            CASE 
                WHEN (cst.ClaimStatusTypeId is null OR c1.ClaimStatusTransactionId IS NULL) THEN c1.BilledAmount
            END
        ) AS 'InProcessTotals',
        COUNT(DISTINCT( 
            CASE 
                WHEN (cst.ClaimStatusTypeId is null OR c1.ClaimStatusTransactionId IS NULL) THEN c1.ClaimLevelMd5Hash 
            END
        )) AS 'InProcessVisits',
        SUM(
			CASE 

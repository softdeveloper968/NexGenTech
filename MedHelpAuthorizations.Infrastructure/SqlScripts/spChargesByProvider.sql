...
CREATE OR ALTER PROCEDURE [IntegratedServices].[spChargesByProvider]
@ClientId int
...
SELECT 
	...
	--,CONVERT(date, c1.DateOfServiceFrom) as 'ServiceDate'
...
WHERE C1.ClientId = @ClientId
	AND (t.ClaimLineItemStatusId IN (SELECT convert(int, value)
		...
		GROUP BY c1.ClaimLevelMd5Hash, Per.LastName + ', ' + Per.FirstName, Prv.Id 

...
CREATE OR ALTER PROCEDURE [IntegratedServices].[spGetChargesByLocation]
@ClientId int 
...
SELECT 
  ...
  ,Count(DISTINCT(c1.ClaimLevelMd5Hash)) as 'ClaimCount'
...
WHERE C1.ClientId = @ClientId
	AND (t.ClaimLineItemStatusId IN (SELECT convert(int, value)
		...
		GROUP BY ClientLoc.id, ClientLoc.[Name]

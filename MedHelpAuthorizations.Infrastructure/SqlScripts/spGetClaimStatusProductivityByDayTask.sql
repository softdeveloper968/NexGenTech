...
BEGIN
       SELECT 
        ...
        ,min(DATEADD(ww, DATEDIFF(ww, -1, CONVERT(DATE, c1.ClaimBilledOn)), -1)) as FirstDateOfWeekBilledOn
		,min(DATEADD(ww, DATEDIFF(ww, -1, CONVERT(DATE, c1.DateOfServiceFrom)), -1)) as FirstDateOfWeekServiceDate
		,DATEPART(ww, CONVERT(DATE, c1.ClaimBilledOn)) as 'WeekNumberBilledOn'
		,DATEPART(ww, CONVERT(DATE, c1.DateOfServiceFrom)) as 'WeekNumberServiceDate'
		,CASE
			WHEN month(min(CONVERT(DATE, c1.ClaimBilledOn))) = 1 
					AND DATEPART(ww, CONVERT(DATE, c1.ClaimBilledOn)) >= 208
				THEN year(CONVERT(DATE, c1.ClaimBilledOn)) - 4  
			WHEN month(min(CONVERT(DATE, c1.ClaimBilledOn))) = 1 
					AND DATEPART(ww, CONVERT(DATE, c1.ClaimBilledOn)) >= 156
				THEN year(CONVERT(DATE, c1.ClaimBilledOn)) - 3 
			WHEN month(min(CONVERT(DATE, c1.ClaimBilledOn))) = 1 
					AND DATEPART(ww, CONVERT(DATE, c1.ClaimBilledOn)) >= 104
				THEN year(CONVERT(DATE, c1.ClaimBilledOn)) - 2 
			WHEN month(min(CONVERT(DATE, c1.ClaimBilledOn))) = 1 
					AND DATEPART(ww, CONVERT(DATE, c1.ClaimBilledOn)) >= 52
				THEN year(CONVERT(DATE, c1.ClaimBilledOn)) - 1 
			WHEN month(min(CONVERT(DATE, c1.ClaimBilledOn))) = 12
					AND DATEPART(ww, CONVERT(DATE, c1.ClaimBilledOn)) = 1
				THEN year(CONVERT(DATE, c1.ClaimBilledOn)) + 1 
			ELSE year(CONVERT(DATE, c1.ClaimBilledOn))
		END as 'WeekYearBilledOn'
		,CASE
			WHEN month(min(CONVERT(DATE, c1.DateOfServiceFrom))) = 1 
					AND DATEPART(ww, CONVERT(DATE, c1.DateOfServiceFrom)) >= 208
				THEN year(CONVERT(DATE, c1.DateOfServiceFrom)) - 4  
			WHEN month(min(CONVERT(DATE, c1.DateOfServiceFrom))) = 1 
					AND DATEPART(ww, CONVERT(DATE, c1.DateOfServiceFrom)) >= 156
				THEN year(CONVERT(DATE, c1.DateOfServiceFrom)) - 3 
			WHEN month(min(CONVERT(DATE, c1.DateOfServiceFrom))) = 1 
					AND DATEPART(ww, CONVERT(DATE, c1.DateOfServiceFrom)) >= 104
				THEN year(CONVERT(DATE, c1.DateOfServiceFrom)) - 2 
			WHEN month(min(CONVERT(DATE, c1.DateOfServiceFrom))) = 1 
					AND DATEPART(ww, CONVERT(DATE, c1.DateOfServiceFrom)) >= 52
				THEN year(CONVERT(DATE, c1.DateOfServiceFrom)) - 1 
			WHEN month(min(CONVERT(DATE, c1.DateOfServiceFrom))) = 12
					AND DATEPART(ww, CONVERT(DATE, c1.DateOfServiceFrom)) = 1
				THEN year(CONVERT(DATE, c1.DateOfServiceFrom)) + 1 
			ELSE year(CONVERT(DATE, c1.DateOfServiceFrom)) 
    ...
    WHERE i.ClientId = @ClientId
        ...
        --AND ((CONVERT(DATE, c1.DateOfServiceFrom) >= @DateOfServiceFrom) OR @DateOfServiceFrom is NULL)
        --AND ((c1.DateOfServiceTo <= @DateOfServiceTo) OR @DateOfServiceTo is NULL)			
        --AND ((CONVERT(DATE, c1.ClaimBilledOn) >= @ClaimBilledFrom) OR @ClaimBilledFrom IS NULL)
        --AND ((CONVERT(DATE, c1.ClaimBilledOn) <= @ClaimBilledTo) OR @ClaimBilledTo is null)
		AND c1.IsDeleted = 0 
        AND c1.IsSupplanted = 0
		

    GROUP BY i.LookupName,  c1.ProcedureCode, (Per.LastName + ',' + per.FirstName)
	, ClientLoc.Name, CONVERT(DATE, c1.DateOfServiceFrom), CONVERT(DATE, c1.ClaimBilledOn), t.ClaimLineItemStatusId, t.ClaimStatusExceptionReasonCategoryId 

...
CREATE OR ALTER PROCEDURE [IntegratedServices].[spGetClaimStatusRevenueTotals]
    @ClientId int = NULL
...
AS
BEGIN;
SELECT s.Code as 'ClaimLineItemStatus'
        ...
        ,CONVERT(date, c1.ClaimBilledOn) as 'Billed On'
		,CONVERT(date, c1.DateOfServiceFrom) as 'DOS From'
...
WHERE C1.ClientId = @ClientId
	AND (t.ClaimLineItemStatusId IN (SELECT convert(int, value)
        ...
        --AND ((c1.DateOfServiceFrom >= @ClaimBilledFrom) OR (c1.ClaimBilledOn >= @ClaimBilledFrom))
        --AND ((c1.DateOfServiceTo <= @ClaimBilledTo) OR (c1.ClaimBilledOn <= @ClaimBilledTo))

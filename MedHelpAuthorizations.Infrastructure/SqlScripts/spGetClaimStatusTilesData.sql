...
CREATE OR ALTER   PROCEDURE [IntegratedServices].[spGetClaimStatusVisits]
	@ClientId int
...
Select 
	...
	,COUNT( DISTINCT(
				CASE 
					WHEN (cst.ClaimStatusTypeId = 2 AND (t.WriteoffAmount = 0 OR t.WriteoffAmount is NULL))  -- Denial Types
					THEN c1.ClaimLevelMd5Hash
				END
				)) AS 'DenialVisits'
	,SUM(
	...
	,SUM(
				CASE 
					WHEN (cst.ClaimStatusTypeId = 1 AND t.TotalAllowedAmount > t.LineItemPaidAmount)  -- Paid Types Where Allowed > Paid Amount
					THEN (t.TotalAllowedAmount - t.LineItemPaidAmount)
				END
				) AS 'Sec/Ps'
...
JOIN(
	...
	WHERE C1.ClientId = @ClientId
		AND (cst.ClaimStatusTypeId = 1 OR (t.ClaimLineItemStatusId IN (SELECT convert(int, value)

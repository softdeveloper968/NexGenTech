...
GO
CREATE OR ALTER   PROCEDURE [IntegratedServices].[spGetClaimStatusTotalsTaskNew]
@ClientId int
...
WHERE C1.ClientId = @ClientId
	AND (t.ClaimLineItemStatusId IN (SELECT convert(int, value)
		...
		--, ClientLoc.Name, ClientLoc.Npi
		--, er.Code
		--, c1.ClaimLevelMD5hash
		GROUP BY t.ClaimLineItemStatusId, c1.ClaimLevelMD5hash
	END

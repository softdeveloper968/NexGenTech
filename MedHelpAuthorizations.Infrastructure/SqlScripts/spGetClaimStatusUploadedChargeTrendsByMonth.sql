...
CREATE or ALTER PROCEDURE [IntegratedServices].[spGetClaimStatusUploadedChargeTrendsByMonth]
	@ClientId int
...
BEGIN
 ...
 WHERE i.ClientId = @ClientId
    ...
    GROUP BY  dateadd(month, datediff(month, 0, (CASE WHEN @DateOfServiceFrom is NOT NULL THEN c1.DateOfServiceFrom ELSE c1.ClaimBilledOn END)), 0), c1.ClaimLevelMD5Hash
    ORDER BY 'TrendingDate'
-- GROUP BY i.LookupName, a.Name, c1.ProcedureCode, dateadd(month, datediff(month, 0, (CASE WHEN @DateOfServiceFrom is NOT NULL THEN c1.DateOfServiceFrom ELSE c1.ClaimBilledOn END)), 0)  
-- ORDER BY 'TrendingDate',i.LookupName, a.Name, c1.ProcedureCode
END
GO

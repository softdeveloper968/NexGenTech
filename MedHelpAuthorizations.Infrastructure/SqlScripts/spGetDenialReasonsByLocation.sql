...
GO
CREATE OR ALTER  PROCEDURE [IntegratedServices].[spGetDenialReasonsByLocation]
@ClientId int
...
SELECT	
		...
		,claimStatusTransaction.ClaimStatusExceptionReasonCategoryId as 'ExceptionReasonCategoryId'
...
WHERE claimStatusBatchClaim.ClientId = @ClientId
	   ...
	   AND (claimLineItemStatus.ClaimStatusTypeId = 2 AND (claimStatusTransaction.WriteoffAmount IS NULL OR claimStatusTransaction.WriteoffAmount = 0))
		...
		GROUP BY claimStatusTransaction.ClaimStatusExceptionReasonCategoryId
		, clientLocation.[Name]
		, clientLocation.Id

USE [faDevDb]
GO
/****** Object:  StoredProcedure [IntegratedServices].[spGetDenialsByProcedure]    Script Date: 11/21/2024 5:33:21 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER   PROCEDURE [IntegratedServices].[spGetDenialsByProcedure]
	@ClientId int
...
SELECT 
		...
		,SUM(
		CASE 
			WHEN (cst.ClaimStatusTypeId = 2 AND (t.WriteoffAmount = 0 OR t.WriteoffAmount is NULL)) THEN c1.BilledAmount
		END
			) AS 'ChargedSum'
...
WHERE C1.ClientId = @ClientId AND Prv.Id > 0
 ...
 GROUP BY 
		...
		,TRIM(c1.[ProcedureCode]), c1.ClaimLevelMd5Hash
END
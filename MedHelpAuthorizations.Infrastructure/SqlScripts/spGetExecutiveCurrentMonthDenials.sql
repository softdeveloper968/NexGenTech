...
CREATE OR ALTER PROCEDURE [IntegratedServices].[spGetExecutiveCurrentMonthDenials]
...
SELECT 
		...
		,SUM(CASE WHEN c1.ClaimBilledOn >= @FromDate AND c1.ClaimBilledOn <= @ToDate THEN c1.BilledAmount ELSE 0 END) AS 'Denials'
		,AVG(CASE WHEN c1.ClaimBilledOn >= @FromDate AND c1.ClaimBilledOn <= @ToDate THEN c1.BilledAmount ELSE 0 END) AS 'AvgDenials'

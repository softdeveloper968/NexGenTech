...
CREATE OR ALTER   PROCEDURE [IntegratedServices].[spGetPaymentTotalsbyPayer]
@ClientId int
...
SELECT 
		...
		--,SUM([claimStatusBatchClaim].BilledAmount) as 'PaidAmountSum'
		,SUM([claimStatusTransaction].LineItemPaidAmount) as 'PaidAmountSum'--EN-620
...
JOIN(
	...
	LEFT JOIN [dbo].ClientInsurances as [clientInsurance] on [clientInsurance].Id = [claimStatusBatchClaim].ClientInsuranceId
...
WHERE [claimStatusBatchClaim].ClientId = @ClientId
	AND ([claimStatusTransaction].ClaimLineItemStatusId IN (SELECT convert(int, value)
		...
		GROUP BY [clientInsurance].Id
		, [clientInsurance].LookupName
	END

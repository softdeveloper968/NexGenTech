...
CREATE OR ALTER PROCEDURE [IntegratedServices].[spGetProviderDenialReasonTotal]
	@ClientId int
...
SELECT 
		...
		,CASE WHEN NULLIF(LTRIM(RTRIM(Per.LastName + ',' + per.FirstName)), '') IS NOT NULL 
			THEN Per.LastName + ',' + per.FirstName 
			ELSE NULL 
		 END as 'ClientProviderName'

...
CREATE OR ALTER       PROCEDURE [IntegratedServices].[spPaymentsMonthly]
     @ClientId int
...
SELECT 
	...
	Sum(t.LineItemPaidAmount) as 'PaymentTotals'
	,Count(c1.ClaimLevelMd5Hash) as 'ClaimCount'
...
WHERE C1.ClientId = @ClientId
	AND (t.ClaimLineItemStatusId IN (SELECT convert(int, value)
		...
		Group by c1.ClaimLevelMd5Hash, CONVERT(DATE, c1.ClaimBilledOn), CONVERT(DATE, c1.DateOfServiceFrom)
	END
GO